...
concept call_hook = hook<Hook> && requires {
	{
		Hook::relocation
	} -> std::convertible_to<REL::RelocationID>;
	{
		Hook::offset
...
namespace Hooking
{
	...
	void write_thunk()
	{
		const REL::Relocation<std::uintptr_t> rel{ Hook::relocation, static_cast<std::ptrdiff_t>(Hook::offset) };
		...
		if (opcode == 0xE8) {  // CALL instruction
			...
			if ((opcode & rexw) != rexw) {  // REX.W Must be present for a valid 64-bit address replacement.
				LOG(critical, "Invalid hook location, lea instruction must use 64-bit register (first byte should be between 0x48 and 04F)"sv);
			    return;
			}
			...
			if (opcode == 0x8D) {                  // LEA instruction
				...
				LOG(critical, "Invalid hook location, write_thunk can only be used for call or lea instructions"sv);
			}
		}
	}
	...
	void install_hook()
	{
		...
		if constexpr (call_hook<Hook>) {
			Hooking::write_thunk<Hook>();
		} else if constexpr (vtable_hook<Hook>) {
			Hooking::write_vfunc<Hook>();
		} else {
